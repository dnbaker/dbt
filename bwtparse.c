...
int main(int argc, char *argv[])
{
...
#if _64BIT_BWT
  uint64_t *BWT = (uint64_t *)realloc(Text, (n + 1) * sizeof(uint64_t));
  if(BWT==NULL) die("realloc failed (BWT)");
#else
  uint32_t *BWT = Text;
#endif
  for(long i=0;i<=n;i++)
  ...
  uint32_t
#endif
        *occ = malloc((k+1)*sizeof(*occ)); // extra space for the only occ of 0
